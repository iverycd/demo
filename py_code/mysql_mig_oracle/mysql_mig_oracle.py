# -*- coding: utf-8 -*-
# mysql_mig_oracle.py v1.0 2021.01.25
...
MySQLPOOL = PooledDB(
    ...
    setsession=['SET AUTOCOMMIT=1;'],  # 开始会话前执行的命令列表。使用连接池执行dml，这里需要显式指定提交，已测试通过
    ping=0,
...
parser = argparse.ArgumentParser(prog='mysql_mig_kingbase',
                                 formatter_class=argparse.RawDescriptionHelpFormatter,
                                 description=textwrap.dedent('''\
EXAMPLE:
    eg(1):perform migration fetch and insert 10000 rows data into table:\n ./mysql_mig_kingbase -b 10000\n
    eg(2):perform custom table migration to kingbase:\n ./mysql_mig_kingbase -c true'''))
parser.add_argument('--batch_size', '-b', help='fetch and insert row size,default 10000', type=int)
parser.add_argument('--custom_table', '-c', help='mig some tables not all tables into kingbase,default false',
...
def tbl_columns(table_name):
    ...
    sql = """select column_name,data_type,character_maximum_length,is_nullable,column_default,numeric_precision,numeric_scale,datetime_precision,column_key,column_comment from information_schema.COLUMNS where table_schema in (select database()) and table_name='%s'""" % table_name
    ...
    for column in output_table_col:  # 按照游标行遍历字段
        ...
        #                }
        #               )
        if column[1].upper() in list_varchar:  # mysql中普通字符串类型均映射为kingbase的普通类型字符串
            if column[2] >= 1300:
                result.append({'column_name': column[0],  # 如下为字段的名称
        ...
        elif column[1].upper() in list_text:  # mysql中所有text类型均映射为kingbase的clob
            result.append({'column_name': column[0],  # 如下为字段的名称
        ...
        #  mysql整数类型映射
        elif column[1].upper() in list_int:  # mysql中MEDIUMINT类型均映射为kingbase的int
            result.append({'column_name': column[0],  # 如下为字段的名称
        ...
        #  mysql非整数类型数字映射
        #  mysql的decimal,double,float均映射为kingbase的decimal
        elif column[1].upper() in list_non_int:  # mysql中MEDIUMINT类型均映射为kingbase的int
            result.append({'column_name': column[0],  # 如下为字段的名称
                           'data_type': 'NUMBER' if str(column[6]).upper() == 'NONE'
                           else 'NUMBER' + '(' + str(column[5]) + ',' + str(column[6]) + ')',
                           # 如果有精度指定精度，否则如果numeric_scale为null，直接映射为kingbase的DECIMAL
                           'character_maximum_length': column[2],
                           'is_nullable': column[3],  # 是否为空
                           'column_default': column[4],  # 字段默认值
                           'numeric_precision': column[5],
        ...
        # mysql时间类型映射
        elif column[1].upper() in list_time:  # mysql中datetime,timestamp映射为kingbase的timestamp
            result.append({'column_name': column[0],  # 如下为字段的名称
                           'data_type': 'date',
                           'character_maximum_length': column[2],
                           'is_nullable': column[3],  # 是否为空
        ...
        # mysql大字段类型映射
        elif column[1].upper() in list_lob:  # mysql中二进制大字段映射为kingbase的bytea
            result.append({'column_name': column[0],  # 如下为字段的名称
                           'data_type': 'BLOB',
                           'character_maximum_length': column[2],
                           'is_nullable': column[3],  # 是否为空
...
def create_meta_table():
    ...
    global all_table_count  # 将kingbase源表总数存入全局变量
    ...
    for row in output_table_name:
        ...
        for struct in structs:
            ...
            if comment_value:  # kingbase中无法在建表的时候添加comment
...
def mig_table(tablename):
    ...
    get_column_length = 'select count(*) from information_schema.COLUMNS where table_schema=' + "'" + source_database + "'" + 'and table_name= ' + "'" + source_table.upper() + "'"  # 拼接获取源表有多少个列的SQL
    ...
    with open("/tmp/table.txt", "r") as f:  # 读取自定义表
        for table_name in f.readlines():  # 按顺序读取每一个表
            ...
            try:
                ...
                f.close()
                # mysql_cursor.execute("""insert into my_mig_task_info(table_name, task_start_time,thread,is_success) values ('%s',current_timestamp,%s,'CHECK TABLE')""" % (table_name, 100))  # %s占位符的值需要引号包围
            val_str = ''  # 用于生成批量插入的列字段变量
            for i in range(1, col_len):
                val_str = val_str + ':' + str(i) + ','
            val_str = val_str + ':' + str(col_len)  # Oracle批量插入语法是 insert into tb_name values(:1,:2,:3)
            ...
            try:
                ...
                print(traceback.format_exc())
            while True:
                rows = list(mysql_cursor.fetchmany(20000))
                # 例如每次获取2000行，cur_oracle_result.arraysize值决定
                # MySQL fetchmany 返回的是 tuple 数据类型 所以用list做类型转换
                # print(cur_oracle_result.description)  # 打印kingbase查询结果集字段列表以及类型
                try:
...
def mig_table_task(list_index):
    ...
    if list_index in list_number:
        ...
        oracle_conn = cx_Oracle.connect(
            oracle_user + '/' + oracle_passwd + '@' + oracle_host + ':' + oracle_port + '/' + service_name)
        oracle_cursor = oracle_conn.cursor()
        oracle_cursor.arraysize = row_batch_size  # kingbase数据库游标对象结果集返回的行数即每次获取多少行
...
def async_work():  # 异步不阻塞方式同时插入表
    ...
    # try:
    #     oracle_cur.execute(
    #         """update my_mig_task_info set run_time=(round(extract(epoch FROM (task_end_time - (task_start_time-interval '1 second') )))::int)""")
    #     oracle_conn.commit()
    # except Exception:
    ...
    # writer = csv.writer(csv_file)
    # oracle_cur.execute("""select table_name, concat('''',cast(task_start_time as char(20)),''''),
    ...
    # mig_task_run_detail = oracle_cur.fetchall()
