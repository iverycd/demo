...
# 读取配置文件
# 以下在pycharm，主程序调用正常，本页文件执行成功，pyinstall生成的可执行文件可读取config文件
# 下面的是针对本次mysql_mig_oracle的目录结构（没有使用configDB.py），与oracle_mig_mysql的目录结构不同
...
# config.read(exepath + "/mysql_mig_oracle/config.ini", encoding='utf-8')
